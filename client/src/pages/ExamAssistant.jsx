...
import {
  Brain, Layers, BookOpen, SlidersHorizontal,
  ChevronLeft, ChevronRight, RotateCcw, Check, X,
  Sparkles, Clock, BarChart2, FileText,
} from 'lucide-react';
...
// ── Constants ─────────────────────────────────────────────────
const MODES = [
  { key: 'mcq',        label: 'Multiple Choice', icon: '🔵', desc: 'Classic MCQ exam with 4 options per question' },
  { key: 'blanks',     label: 'Fill in Blanks',  icon: '✏️', desc: 'Complete sentences with missing words' },
  { key: 'flashcards', label: 'Flashcards',       icon: '🃏', desc: 'Flip cards for active recall practice' },
...
function MCQExam({ questions }) {
  ...
  const reveal = () => setRevealed((r) => ({ ...r, [current]: true }));
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div>
          ...
          <p className="text-ink/50 dark:text-white/40 mt-1">{correct} / {total} correct</p>
        </div>
        <p className="text-sm text-ink/55 dark:text-white/45 max-w-xs">
          {pct >= 80 ? 'Excellent! You have a strong grasp of this topic.' :
           pct >= 50 ? 'Good effort. Review the questions you missed.' :
           'Keep studying — try again after reviewing your notes.'}
        </p>
        <button onClick={() => { setCurrent(0); setSelected({}); setRevealed({}); setFinished(false); }}
          className="btn-primary flex items-center gap-2">
          <RotateCcw size={15} /> Retry exam
        </button>
      </motion.div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs font-semibold text-ink/40 dark:text-white/35">
          Question {current + 1} of {total}
        </span>
        <div className="flex gap-1">
          {questions.map((_, i) => (
            <div key={i} className="h-1.5 w-6 rounded-full transition-all"
              style={{ background: i < current ? '#7C6AF0' : i === current ? '#7C6AF0' : 'rgba(124,106,240,0.15)', opacity: i === current ? 1 : i < current ? 0.6 : 0.3 }} />
          ))}
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={current}
          ...
          className="p-7" style={cardGlass}>
          <p className="font-display font-bold text-ink dark:text-white text-lg mb-6 leading-snug">
            {q.question}
          </p>
            ...
            {q.options.map((opt, i) => {
              ...
              return (
                <button key={i} onClick={() => choose(i)}
                  className="flex items-center gap-3 rounded-2xl px-5 py-3.5 text-left text-sm font-medium transition-all"
                  ...
                  {opt}
                  {isRevealed && isCorrect && <Check size={16} className="ml-auto text-sage-500 shrink-0" />}
                  {isRevealed && isSelected && !isCorrect && <X size={16} className="ml-auto text-coral-500 shrink-0" />}
                </button>
              );
            })}
          </div>
          ...
          {revealed[current] && q.explanation && (
            <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
              className="rounded-2xl px-4 py-3 mb-4 text-sm text-ink/60 dark:text-white/50"
          ...
          <div className="flex gap-3">
            {selected[current] !== undefined && !revealed[current] && (
              <button onClick={reveal}
                className="flex-1 rounded-2xl py-2.5 text-sm font-semibold text-lavender-600 dark:text-lavender-300"
                style={{ background: 'rgba(124,106,240,0.10)', border: '1px solid rgba(124,106,240,0.20)' }}>
                Check answer
              </button>
            )}
            {revealed[current] && (
              <button onClick={next} className="flex-1 btn-primary justify-center">
  ...
  if (finished) {
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="text-7xl">{pct >= 80 ? '🎉' : pct >= 50 ? '💪' : '📚'}</div>
        <h2 className="font-display text-3xl font-bold text-ink dark:text-white">{pct}%</h2>
        <p className="text-ink/50 dark:text-white/40">{correct} / {questions.length} correct</p>
        <button onClick={() => { setAnswers({}); setChecked({}); setFinished(false); }}
          className="btn-primary flex items-center gap-2">
  ...
  return (
    <div className="max-w-2xl mx-auto flex flex-col gap-4">
      {questions.map((q, i) => {
        ...
        return (
          <div key={i} className="p-5" style={cardGlass}>
            <p className="text-sm text-ink/50 dark:text-white/35 mb-2">Question {i + 1}</p>
                ...
                <span key={j}>
                  ...
                  {j < arr.length - 1 && (
                    <input
                      ...
                      style={{
                        borderColor: isChecked ? (isCorrect ? '#4CC38A' : '#FF7A63') : '#7C6AF0',
                        color:       isChecked ? (isCorrect ? '#2DA76E' : '#FF7A63') : '#7C6AF0',
                      }}
                      placeholder="___"
                    />
                  )}
            ...
            {q.hint && !isChecked && (
              <p className="text-xs text-ink/35 dark:text-white/25 mb-3">💡 Hint: {q.hint}</p>
            )}
            {isChecked && (
            ...
            {!isChecked && (
              <button onClick={() => setChecked((c) => ({ ...c, [i]: true }))}
                disabled={!answers[i]?.trim()}
                className="mt-2 text-xs font-semibold text-lavender-600 disabled:opacity-40">
                Check →
              </button>
            )}
          </div>
      ...
      <div className="relative h-64 cursor-pointer mb-6" onClick={() => setFlipped((f) => !f)}
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center"
            ...
            <p className="font-display font-bold text-ink dark:text-white text-lg leading-snug">{card.front}</p>
            <p className="text-xs text-ink/30 dark:text-white/25 mt-4">Tap to reveal answer</p>
          </div>
          {/* Back */}
          <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center"
            ...
            <p className="text-ink dark:text-white leading-relaxed">{card.back}</p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="flex gap-2 justify-center mt-5">
        {cards.map((_, i) => (
          <div key={i} className="h-1.5 w-5 rounded-full transition-all"
            style={{
              background: known.has(i) ? '#4CC38A' : i === current ? '#7C6AF0' : 'rgba(124,106,240,0.15)',
            }} />
        ))}
      </div>
    </div>
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {/* Title */}
          <h2 className="font-display text-2xl font-bold text-ink dark:text-white mb-6 leading-snug">
            {slide.title}
          </h2>
          ...
          {/* Speaker note */}
          {slide.note && (
            <div className="mt-6 pt-4 border-t border-ink/8 dark:border-white/8">
              <p className="text-xs text-ink/35 dark:text-white/25 italic">
                📝 Note: {slide.note}
              </p>
            </div>
          )}
      ...
      <div className="flex items-center justify-between mt-4">
        <button onClick={() => setCurrent((c) => Math.max(0, c - 1))}
          disabled={current === 0}
          className="flex items-center gap-2 rounded-2xl px-5 py-2.5 text-sm font-semibold disabled:opacity-30 transition"
          style={glass}>
          <ChevronLeft size={16} /> Previous
        </button>
        <span className="text-xs text-ink/40 dark:text-white/30 font-medium">
...
export default function ExamAssistant() {
  ...
  const generate = async () => {
    if (!notes.trim()) { toast.error('Paste your notes or describe a topic first.'); return; }
    setLoading(true);
    setResult(null);
...
CRITICAL: Cover ALL topics from the notes. Do not skip any concept.
Return ONLY a valid JSON array. Each object must have:
- question (string)
- options (array of exactly 4 strings)
- correct (number, 0-indexed position of correct answer)
...
CRITICAL: Cover ALL key terms and concepts from the notes.
Return ONLY a valid JSON array. Each object must have:
- sentence (string with ___ where the blank should be)
- answer (string, the correct word/phrase)
- hint (string, a subtle clue)

Notes:
${notes}`;
    } else if (mode === 'flashcards') {
      prompt = `Generate exactly ${count} flashcards based on these notes.
CRITICAL: Every important concept, term, date, formula, or fact must appear as a card. Do not skip anything.
Return ONLY a valid JSON array. Each object must have:
- front (string, the question or term)
- back (string, the complete answer or definition)

Notes:
${notes}`;
    } else if (mode === 'slides') {
      prompt = `Create a comprehensive slide deck based on these notes.
CRITICAL REQUIREMENT: You MUST include 100% of the information from the notes. Do not summarize, compress, or omit ANY detail, example, date, formula, name, or concept. If the notes are long, create more slides. Every piece of information must appear somewhere in the deck.
Generate as many slides as needed to cover everything.
Return ONLY a valid JSON array. Each object must have:
- title (string, slide title)
- bullets (array of strings, detailed bullet points — include full explanations, not just keywords)
- note (string, additional context or detail for this slide)
...
${notes}`;
    ...
    try {
      ...
      toast.success(`${modeLabel} generated! ${parsed.length} ${mode === 'slides' ? 'slides' : mode === 'flashcards' ? 'cards' : 'questions'}`);
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Paste your notes — Lumi generates exams, flashcards, and slides instantly."
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 flex flex-col gap-4">
            ...
            <div className="rounded-3xl p-5" style={glass}>
              <p className="text-xs font-bold uppercase tracking-widest text-ink/40 dark:text-white/30 mb-3">
                Study type
              </p>
                ...
                {MODES.map((m) => (
                  <button key={m.key} onClick={() => setMode(m.key)}
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-ink/40 dark:text-white/30 mt-0.5">{m.desc}</p>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="rounded-3xl p-5" style={glass}>
              <div className="mb-4">
                <p className="text-xs font-bold uppercase tracking-widest text-ink/40 dark:text-white/30 mb-2">
                  {mode === 'slides' ? 'Min slides' : mode === 'flashcards' ? 'Cards' : 'Questions'}: {count}
                </p>
                <input type="range" min={mode === 'slides' ? 5 : 5} max={mode === 'slides' ? 30 : 30}
                  value={count} onChange={(e) => setCount(Number(e.target.value))}
                  className="w-full accent-lavender-600" />
                <div className="flex justify-between text-[10px] text-ink/30 mt-1">
                  <span>{mode === 'slides' ? 5 : 5}</span>
                  <span>{mode === 'slides' ? 30 : 30}</span>
                </div>
              </div>

              {(mode === 'mcq' || mode === 'blanks') && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest text-ink/40 dark:text-white/30 mb-2">
          ...
          <div className="lg:col-span-2 flex flex-col gap-4">
            <div className="rounded-3xl p-6 flex flex-col gap-4 flex-1" style={glass}>
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-ink/40 dark:text-white/30 mb-2">
                  Your notes or topic
                </p>
                <textarea
                  className="w-full rounded-2xl p-4 text-sm text-ink dark:text-white bg-white/60 dark:bg-white/[0.05] border border-white/65 dark:border-white/10 outline-none resize-none placeholder:text-ink/30 dark:placeholder:text-white/25 focus:border-lavender-400 focus:ring-2 focus:ring-lavender-200/50 transition"
                  rows={14}
                  placeholder={`Paste your lecture notes, textbook content, or just describe a topic here…

Examples:
- "Chapter 4: Photosynthesis — the process by which plants convert light energy…"
- "JavaScript closures, async/await, event loop…"
- "The French Revolution: causes, key events, consequences…"
              ...
              {notes.trim() && (
                <div className="flex gap-3 flex-wrap">
                  {[
                    ...
                    <span key={label} className="flex items-center gap-1.5 text-[11px] text-ink/45 dark:text-white/35 font-medium">
                      {icon} {label}
                    </span>
                  ))}
                </div>
        ...
        <div>
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={reset}
              className="flex items-center gap-2 rounded-2xl px-4 py-2 text-sm font-semibold text-ink/55 dark:text-white/45 transition"
              style={glass}>
