      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        ...
        <Chart title={t('analytics.focusWeek')} delay={0.15}>
          <AreaChart data={data.studyHoursPerWeek}>
            ...
            <XAxis dataKey="week" {...axisProps} />
            <YAxis {...axisProps} />
            <Tooltip {...tooltipStyle} formatter={(v) => [`${v}h`, t('analytics.hours')]} />
        ...
        <Chart title={t('analytics.moodTrend')} delay={0.2}>
          <LineChart data={data.moodTrend}>
            ...
            <Tooltip {...tooltipStyle} formatter={(v) => [MOOD_LABELS[v] ? `${MOOD_LABELS[v]} (${v}/5)` : v, t('analytics.moodLabel')]} />
        ...
        <Chart title={t('analytics.prodTrend')} delay={0.25}>
          <AreaChart data={data.productivityTrend}>
            ...
            <Tooltip {...tooltipStyle} formatter={(v) => [`${v}%`, t('analytics.score')]} />
        ...
        <GlassCard delay={0.30} className="p-6">
          ...
          {!data.tasksByCategory?.length ? (
            <div className="flex flex-col items-center justify-center h-[220px] text-center gap-2">
              ...
              <p className="text-sm text-ink/40 dark:text-white/30">{t('analytics.catEmpty')}</p>
            </div>
          ) : (
            <div className="flex items-center gap-4">
              <ResponsiveContainer width="60%" height={220}>
                <PieChart>
                  <Pie data={data.tasksByCategory} dataKey="c" nameKey="category" innerRadius={55} outerRadius={85} paddingAngle={3}>
              ...
              <div className="flex flex-col gap-2 flex-1 min-w-0">
                {data.tasksByCategory.map((entry, idx) => (
                  <div key={entry.category} className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs font-bold text-ink/45 dark:text-white/35 ms-auto shrink-0">{entry.c}</span>
                  </div>
                ))}
              </div>
            </div>
