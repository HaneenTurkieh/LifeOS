...
export default function ProductivitySphere({ score = 0, size = 132 }) {
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <svg width={size} height={size} className="-rotate-90 relative z-10">
        ...
        <motion.circle
          cx={size / 2} cy={size / 2} r={radius} stroke="url(#sphereGrad)" strokeWidth="8" fill="none"
      ...
      <div className="absolute z-10 flex flex-col items-center">
        <span className="font-display text-2xl font-bold text-ink dark:text-white">{score}%</span>
        <span className="text-[10px] text-ink/45 dark:text-white/40">today</span>
      </div>
    </div>
  );
}
