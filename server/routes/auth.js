...
const {
  ...
  authenticate, generateResetToken, hashResetToken,
} = require('../lib/auth');
...
router.post('/register', async (req, res) => {
  ...
  try {
    const existing = await db.execute({ sql: `SELECT id FROM users WHERE email = ? COLLATE NOCASE`, args: [normalizedEmail] });
    ...
    const user = (await db.execute({ sql: `SELECT * FROM users WHERE id = ?`, args: [Number(insert.lastInsertRowid)] })).rows[0];
    res.status(201).json({ token: signToken(user), user: publicUser(user) });
  } catch (err) {
    if (err.message?.includes('UNIQUE constraint')) return res.status(409).json({ error: 'An account with that email already exists' });
...
router.patch('/me', authenticate, async (req, res) => {
  const { name, gender, birthday, bio } = req.body;
  let   { avatar } = req.body;

  // Limit avatar size — base64 images only up to 300KB
  if (avatar && avatar.length > 400000) {
    return res.status(400).json({ error: 'Avatar image is too large. Please use an image under 300KB.' });
  }
  ...
  try {
    ...
    await db.execute({ sql: `UPDATE users SET ${setClauses.join(', ')} WHERE id = ?`, args });
    const result = await db.execute({ sql: `SELECT * FROM users WHERE id = ?`, args: [req.user.id] });
...
router.post('/me/password', authenticate, async (req, res) => {
  ...
  try {
    ...
    await db.execute({ sql: `UPDATE users SET password_hash = ? WHERE id = ?`, args: [password_hash, req.user.id] });
