...
const today = req.query.date || new Date().toISOString().slice(0, 10);
    ...
    // source != 'aurora' everywhere here — the self-seeded birthday
    ...
    const [todaysTasksResult, habitsResult, upcomingResult, moodResult, tasksDoneResult, totalTasksResult, birthdayResult] = await Promise.all([
      db.execute({ sql: `SELECT * FROM tasks WHERE user_id = ? AND status != 'done' AND project_id IS NULL AND source != 'aurora' AND (deadline = ? OR deadline IS NULL) ORDER BY priority DESC LIMIT 6`, args: [userId, today] }),
      db.execute({ sql: `SELECT * FROM habits WHERE user_id = ?`, args: [userId] }),
      db.execute({ sql: `SELECT * FROM tasks WHERE user_id = ? AND status != 'done' AND project_id IS NULL AND source != 'aurora' AND deadline IS NOT NULL AND deadline >= ? ORDER BY deadline ASC LIMIT 5`, args: [userId, today] }),
      ...
      db.execute({ sql: `SELECT COUNT(*) c FROM tasks WHERE user_id = ? AND status='done' AND project_id IS NULL AND source != 'aurora' AND deadline = ?`, args: [userId, today] }),
      db.execute({ sql: `SELECT COUNT(*) c FROM tasks WHERE user_id = ? AND project_id IS NULL AND source != 'aurora' AND deadline = ?`, args: [userId, today] }),
      db.execute({ sql: `SELECT title, deadline FROM tasks WHERE user_id = ? AND source = 'aurora' AND category = 'Birthday' LIMIT 1`, args: [userId] }),
    ]);
