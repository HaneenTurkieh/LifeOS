...
function generateOccurrenceDates(recurrence, fromDeadline, untilDate) {
  ...
  while (dates.length < MAX_OCCURRENCES) {
    cursor = nextRecurrenceDate(recurrence, cursor);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (!wasDone && isNowDone) {
      ...
      // recurrence_until excluded here on purpose — a bounded series
      // (has an end date) is already fully pre-created either at
      // create time or by recurrenceJustSet above, so spawning another
      // row here on completion would just duplicate an occurrence that
      // already exists. Only open-ended recurrences (no end date, so
      // they couldn't be pre-created) still spawn their next row here,
      // one at a time, same as before.
      if (updates.recurrence && !updates.recurrence_until && !alreadyEarnedXp && !updates.is_birthday) {
    ...
    // most of the time.
    res.json({ task, xpAwarded, nextTask, unlocked: [], late, spawnedOccurrences });
